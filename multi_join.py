...
class MultiTableJoin:
    ...
    def __init__(self,table_num,helper,dataset,boundary,block_size,join_attr,used_dims):
        ...
        for tid, join_q_amount in enumerate(self.q_amount_tables[:table_num]):
            ...
            # join_set,_=helper.generate_queryset_and_save(int(self.q_amount_tables[tid]),queryset_type=self.setting_dict['q_type'][tid])
            join_set=[]
        ...
        for link in range(table_num - 1):
            for target in range(link + 1, table_num):
                ...
                link_join_queries, target_join_queries = ju.generate_join_queries(link_training_set_for_join,
                                                                                  target_training_set_for_join,int(self.q_join_amount_tables[link]/2))
    ...
    def save_queryset(self,base_path=''):
    ...
    def save_tree_model(self,best_depth_tables):
    ...
    def save_tree_model_for_adaptdb(self):
        depth=3
        partition_base_path = '/home/liupengju/pycharmProjects/NORA_JOIN_SIMULATION/PartitionLayout/multi_join/'
        for tid in self.training_set_tables.keys():
